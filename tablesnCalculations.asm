# MIPS-Assembly
# The code is written in the Mars MIPS simulator.
# This is a program that creates 3 tables, the first one is filled with numbers from 11 to 20, the second one is filled with random numbers from 30 to 80 and the third one is filled with the sum of the first two tables.			
...
Loop:		slt $t2, $t1 ,$a1			# if($t1 < $a1) then $t2 <- 1, if not $t2 <- 0
			...
			addiu $v0, $zero, 1			# service 1 -> print integer
			sll $t2, $t1, 2				# $t2 <- ($t1 << 2)	
...
Loop2:		slt $t2, $t1, $t4			# if($t1 < $t4) then $t2 <- 1, if not $t2 <- 0
			...
			addiu $v0, $zero, 42		# service 42 -> random integer range
